...
def bootstrap_spearman(x, y, n_boot=1000, ci=95, seed=42):
    ...
    if len(valid) == 0:
        # Every bootstrap resample was degenerate (e.g. x is constant/near-constant),
        # so no correlation could be computed on any resample.
        ci_lo, ci_hi, p_neg = np.nan, np.nan, np.nan
    else:
        ci_lo  = np.percentile(valid, alpha)
        ci_hi  = np.percentile(valid, 100 - alpha)
        p_neg  = float(np.mean(valid < 0))
    return {
        "spearman_r": obs_r,
        "ci_lo": ci_lo,
        "ci_hi": ci_hi,
