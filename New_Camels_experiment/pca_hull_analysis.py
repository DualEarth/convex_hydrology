"""
pca_hull_analysis.py  (CAMELS_US)
──────────────────────────────────
...
def build_hull(data):
    try:
        ...
        return hull, tree, vertices
    except Exception as e:
        print(f"  [WARN] Hull failed: {e}")
        return None, None, None


def compute_distances(test_points, ref_data, hull, tree):
    ...
    # Hull distance (GPU nearest vertex, GPU inside check via facet inequalities)
    ...
    if tree is not None:
        ...
        if hull is not None:
            A, c   = hull.equations[:, :-1], hull.equations[:, -1]
            batch2 = 2000
            inside = np.empty(len(test_points), dtype=bool)
            for i in range(0, len(test_points), batch2):
                inside[i:i+batch2] = np.all(test_points[i:i+batch2] @ A.T + c <= 1e-7, axis=1)
            hull_dist[inside] = 0.0
    else:
...
def compute_basin_metrics(basin_data_pca, ref_pca, hull, tree, mahal_threshold):
    all_points = np.vstack(list(basin_data_pca.values()))
    hull_dist, cent_dist, mahal_dist = compute_distances(
        all_points, ref_pca, hull, tree
    )
...
def main():
    ...
    hull, tree, _ = build_hull(hull_data)
    ...
    df = compute_basin_metrics(
        basin_data_pca, ref_pca, hull, tree, MAHAL_THRESHOLD
    )
